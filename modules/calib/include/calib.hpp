...
public:
	//mahdi: https://stackoverflow.com/questions/30344962/square-size-with-asymmetric-circles-grid-opencv says square_size is half the distance between circle centers, so 20 mm is correct
	CirclesGrid(cv::Size board_size = cv::Size(4, 11), double square_size = 10.0);//20.0mm for standard circle board, 4mm for minimized board, 10.0 for half size board
	//CirclesGrid(cv::Size board_size = cv::Size(4, 11), double square_size = 16.62);//mahdi: this is from douglas
	//mahdi: cv::size(width, height) or (number of columns, number of rows)
	bool find(lpt::ImageFrame& frame);
...
public:
	...
	// Static inline create function to instantiate a shared pointer to Calibrator
	static inline lpt::Calibrator::Ptr create(vector<lpt::Camera>& pt_cameras, vector<lpt::CameraPair>& pt_camera_pairs, int& cam_id, string& opath) {
		return lpt::Calibrator::Ptr(new lpt::Calibrator(pt_cameras, pt_camera_pairs, cam_id, opath)); 		
	} 

	// Friend functions for various callbacks related to calibration
	...
	friend void callbackSetWriteCalibImages(int state, void* data);

	// Constructor
	Calibrator(vector<lpt::Camera>& pt_cameras, vector<lpt::CameraPair>& pt_camera_pairs, int& cam_id, string& opath)
		: cameras(pt_cameras), camera_pairs(pt_camera_pairs), window_name("calibration"), frame_interval(40), stereo_frame_max(200), current_camera(cam_id), output_path(opath)
		{ 
			stereo_data_collection = false;
			intparam_data_collection = false;
			writeCalibImages = false;
			get_global_reference = false;
			updated = false;
			object = new lpt::ThreePointLine(); 
			//board = new lpt::Chessboard(cv::Size(9, 6), 25.6); //mahdi: not in douglas code
			board = new lpt::CirclesGrid();
			calib_flags = cv::CALIB_FIX_ASPECT_RATIO | cv::CALIB_FIX_K3 | cv::CALIB_FIX_K4 | cv::CALIB_FIX_K5;
			//calib_flags = cv::CALIB_RATIONAL_MODEL | cv::CALIB_FIX_ASPECT_RATIO;
		}
	...
	inline void setCalibViews(cv::Mat image_type) {
		...
		current_calibration_view = cv::Mat::zeros(image_type.size(), CV_8UC3);
	...
	inline void setFrameInterval(int frameInterval) { frame_interval = frameInterval; }
	inline void setWriteCalibImages(bool state) { writeCalibImages = state; }
	...
	array<double, 3> computeReprojectionErrors(
	...
	void exportResiduals(const vector<vector<cv::Point2f>>& residuals, const string& set_name);
	vector<cv::Mat> calib_views;
	vector<cv::Mat> stereo_views;
	vector<cv::Mat> ref_views;
	cv::Mat current_calibration_view;
private:
	int& current_camera;
