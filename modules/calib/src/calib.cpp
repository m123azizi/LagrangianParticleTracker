...
namespace lpt {
	...
	CirclesGrid::CirclesGrid(cv::Size board_size, double square_size) {
		...
		for (int i = 0; i < board_size.height; i++) { // 0 to 10
			for (int j = 0; j < board_size.width; j++) { // 0 to 3
	...
	void Calibrator::addControls() {
		...
		cv::createButton("save calib imgs", callbackSetWriteCalibImages, calibrator_void_ptr, cv::QT_CHECKBOX, 0);
	...
	bool Calibrator::findCalibBoard(lpt::ImageFrame& frame) {
		...
		if (found) {
			...
			cout << cameras[current_camera].name << " -- " << cameras[current_camera].frames.size()-1 << endl;//this showed one more
			cv::namedWindow(cameras[current_camera].name, cv::WINDOW_NORMAL);
			cv::resizeWindow(cameras[current_camera].name, 800, 640);
			cv::imshow(cameras[current_camera].name, calib_views[current_camera]);
			current_calibration_view.setTo(cv::Scalar(0));
			board->draw(current_calibration_view);
			std::string Blended_filename = output_path + "/calib_camera" + std::to_string(current_camera) + "_" + std::to_string(cameras[current_camera].frames.size()) + "blended.png";
			cv::Mat Blended_image, color_frame_image;
			cv::cvtColor(frame.image, color_frame_image, cv::COLOR_GRAY2BGR);
			double alpha = 0.5; double beta = 1.0 - alpha;
			cv::addWeighted(color_frame_image, alpha, current_calibration_view, beta, 0.0, Blended_image);
			cv::namedWindow("Blended Image", cv::WINDOW_NORMAL);
			cv::resizeWindow("Blended Image", 640, 480);
			cv::imshow("Blended Image", Blended_image);

			// to save images
			if (this->writeCalibImages) {
				std::string filename = output_path + "/calib_camera" + std::to_string(current_camera) + "_" + std::to_string(cameras[current_camera].frames.size()) + ".png";
				cv::imwrite(filename, frame.image);
				cv::imwrite(Blended_filename, Blended_image);
			}
		}
		return found;
	}
	...
	bool Calibrator::findCalibObject(lpt::ImageFrameGroup& group) {
		//mahdi this aims to find an exactly 3 point line in a group of images
		//and then use the line to find the calibration object, by reordering and making sure that perpendicular distabce of the 
		//point 0 to the line connecting point 1 and 2 is smaller than a threshold 1.5
		//then it drwas it and pushes data to stereo_data_frames
		bool allfound = true;
		...
		if (allfound) {
			...
			ofstream fout(output_path + "/3point_calib_object_" + std::to_string(stereo_data_frames.size()) + ".txt");
			for (int i = 0; i < group.size(); ++i) {
				...
				cv::Mat Blended_image, color_frame_image;
				cv::cvtColor(group[i].image, color_frame_image, cv::COLOR_GRAY2BGR);
				double alpha = 0.5; double beta = 1.0 - alpha;
				cv::addWeighted(color_frame_image, alpha, stereo_views[i], beta, 0.0, Blended_image);
				cv::imshow(cameras[i].name, Blended_image);
				cv::imwrite(output_path + "/3point_calib_object_" + std::to_string(stereo_data_frames.size()) + std::to_string(i) + ".png", group[i].image);
				fout << cameras[i].name << endl;
				for (int j = 0; j < group[i].particles.size(); ++j) {
					fout << group[i].particles[j]->x << ", " << group[i].particles[j]->y << "\n";
				}
			}
			fout.close();
		}
	...
	//	vector<vector<cv::Point2f>> reprojection_errors_train;
	//	vector<vector<cv::Point2f>> reprojection_errors_test;
	//	cv::Mat camera_matrix, dist_coeffs;
	//	double total_avg_err = 0;

	//	// For breaking into test and train sets
	//	int half_size = cameras[current_camera].frames.size() / 2;
	//	half_size = 30;
	//	vector<vector<cv::Point2f>> train_image_points(image_points.begin(), image_points.begin() + half_size);
	//	vector<vector<cv::Point3f>> train_object_points(object_points.begin(), object_points.begin() + half_size);
	//	vector<vector<cv::Point2f>> test_image_points(image_points.begin() + half_size, image_points.end());
	//	vector<vector<cv::Point3f>> test_object_points(object_points.begin() + half_size, object_points.end());

	//	// Print sizes for debugging
	//	std::cout << "Total frames: " << cameras[current_camera].frames.size() << std::endl;
	//	std::cout << "Half size: " << half_size << std::endl;
	//	std::cout << "Train image points size: " << train_image_points.size() << std::endl;
	//	std::cout << "Train object points size: " << train_object_points.size() << std::endl;
	//	std::cout << "Test image points size: " << test_image_points.size() << std::endl;
	//	std::cout << "Test object points size: " << test_object_points.size() << std::endl;

	//	vector<cv::Mat> rvecs_train(train_object_points.size()), tvecs_train(train_object_points.size());
	//	vector<cv::Mat> rvecs_test(test_object_points.size()), tvecs_test(test_object_points.size());

	//	double rms = cv::calibrateCamera(train_object_points, train_image_points, image_size, camera_matrix, dist_coeffs, rvecs_train, tvecs_train, calib_flags);

	//	std::cout << "train images RMS error reported by cv::calibrateCamera(): " << rms << std::endl;
	//	bool ok = cv::checkRange(camera_matrix) && cv::checkRange(dist_coeffs);

	//	if (ok) {
	//		std::cout << "Calibration successful." << std::endl;

	//		// Solve PnP for test images
	//		for (size_t i = 0; i < test_image_points.size(); ++i) {
	//			cv::solvePnP(test_object_points[i], test_image_points[i], camera_matrix, dist_coeffs, rvecs_test[i], tvecs_test[i]);
	//		}

	//		array<double, 3> train_reprojection_errors = computeReprojectionErrors(
	//			train_object_points, train_image_points, rvecs_train, tvecs_train, camera_matrix, dist_coeffs, 
	//			reprojection_errors_train, true, output_path, current_camera, 0, " train");

	//		array<double, 3> test_reprojection_errors = computeReprojectionErrors(
	//			test_object_points, test_image_points, rvecs_test, tvecs_test, camera_matrix, dist_coeffs, 
	//			reprojection_errors_test, true, output_path, current_camera, half_size, " test");

	//		double mean_train_error = train_reprojection_errors[0];
	//		double stddev_train_error = train_reprojection_errors[1];
	//		double rms_train_error = train_reprojection_errors[2];
	//		double mean_test_error = test_reprojection_errors[0];
	//		double stddev_test_error = test_reprojection_errors[1];
	//		double rms_test_error = test_reprojection_errors[2];

	//		std::cout << "Mean reprojection error on training set: " << mean_train_error << std::endl;
	//		std::cout << "Standard deviation of reprojection error on training set: " << stddev_train_error << std::endl;
	//		std::cout << "RMS reprojection error on training set: " << rms_train_error << std::endl;
	//		std::cout << "Mean reprojection error on test set: " << mean_test_error << std::endl;
	//		std::cout << "Standard deviation of reprojection error on test set: " << stddev_test_error << std::endl;
	//		std::cout << "RMS reprojection error on test set: " << rms_test_error << std::endl;

	//		exportResiduals(reprojection_errors_train, "train");
	//		exportResiduals(reprojection_errors_test, "test");
	//	}
	//	else {
	...
	//	//exportResiduals(reprojection_errors);

	//	//cout << (ok ? "Calibration succeeded" : "Calibration failed") << " error_stats avg reprojection error = " << error_stats[0] << " +- " << error_stats[1] << endl;
	//	//

	//// Print all the distortion coefficients to check if higher values are close to zero
	//	std::cout << "Distortion Coefficients Size: " << dist_coeffs.size() << std::endl;
	//	std::cout << "Distortion Coefficients: " << std::endl;
	//	for (int i = 0; i < dist_coeffs.rows; i++) {
	//		for (int j = 0; j < dist_coeffs.cols; j++) {
	//			std::cout << dist_coeffs.at<double>(i, j) << " ";
	//		}
	//		std::cout << std::endl;
	//	}
	...
	//				image_size, camera_matrix, dist_coeffs, rvecs_train,
	//				tvecs_train, calib_flags | cv::CALIB_USE_INTRINSIC_GUESS | cv::CALIB_FIX_PRINCIPAL_POINT);
	...
	void Calibrator::exportResiduals(const vector<vector<cv::Point2f>>& residuals, const string& set_name) {
		string out_file_name = output_path + "/calib residuals " + set_name + "_" + to_string(current_camera) + ".csv";
		ofstream fout(out_file_name);
		if (!fout) {
			cout << "Error opening output file for writing calibration residuals: " << out_file_name << endl;
			return;
		}
		fout << "residual_x,residual_y\n";
		for (size_t i = 0; i < residuals.size(); ++i) {
			for (size_t j = 0; j < residuals[i].size(); ++j) {
				fout << residuals[i][j].x << "," << residuals[i][j].y << "\n";
			}
			fout << endl;
		}
		fout.close();
		cout << "Calibration residuals exported to " << out_file_name << endl;
	}
	...
	bool Calibrator::findGlobalReference(lpt::ImageFrameGroup& frame_group) {
		bool allfound = true;
		vector< vector<cv::Point2f > > image_points(frame_group.size());
		for (int c = 0; c < frame_group.size(); ++c) {
			allfound &= board->find(frame_group[c]);
			if (allfound)
				image_points[c] = board->image_points;
			else {
				cout << "Failed to find calibration board in " << cameras[c].name << endl;
				break;
			}
		}
		...
		if (allfound) {			
			for (int c = 0; c < frame_group.size(); ++c) {
				string fileName = output_path + "globRef_cam" + std::to_string(c) + "_" + std::to_string(frame_group[c].frame_index) + ".png";
				cv::imwrite(fileName, frame_group[c].image);
				ref_views[c] = frame_group[c].image.clone();
	...
	array<double, 3> Calibrator::computeReprojectionErrors(
		...
		vector<vector<cv::Point2f>>& per_view_errors,
		bool visualize, const string& output_path, int current_camera, int half_size, const string& set_name)
	{
		//vector<cv::Point2f> image_points2;
		//int total_points = 0;
		...
		//	cv::Mat img;
		//	if (visualize) {
		//		cv::cvtColor(cameras[current_camera].frames[i+half_size+1].image, img, cv::COLOR_GRAY2BGR);
		//	}

		//	for (size_t j = 0; j < object_points[i].size(); ++j) {
		//		cv::Point2f residual(
		//			image_points[i][j].x - image_points2[j].x,
		//			image_points[i][j].y - image_points2[j].y);
		//		per_view_errors[i].push_back(residual);

		//		double pixel_distance = sqrt(residual.x * residual.x + residual.y * residual.y);
		//		error_stats(pixel_distance);
		//		total_error += pixel_distance * pixel_distance;
		//		total_points++;

		//		if (visualize) {
		//			cv::circle(img, image_points[i][j], 5, cv::Scalar(0, 255, 0), 3);  // Original points in green
		//			cv::circle(img, image_points2[j], 5, cv::Scalar(0, 0, 255), 2);    // Reprojected points in red
		//		}
		//	}

		//	if (visualize) {
		//		string img_name = output_path + "/calib" + set_name + " Image " + to_string(current_camera) + "_" + to_string(i) + ".png";
		//		//cv::imshow("Reprojection Errors", img);
		//		cv::imwrite(img_name, img);
		//		//cv::waitKey(100); //this causes the program to wait for key press
		//	}
		//}
		...
		//error[2] = sqrt(total_error / total_points); // RMS error
		//return error;
		return { 0,0,0 };
	}
	...
	void callbackSetStereoDataCollection(int state, void* data) {
		...
		cout << "stereo data collection : " << (state ? " started" : "stopped") << endl;
	}

	void callbackSetIntParamDataCollection(int state, void* data) {
		lpt::Calibrator* calibrator = static_cast<lpt::Calibrator*>(data);
		calibrator->setIntParamDataCollection((state != 0));
		cout << "internal param data collection " << (state? " started" : "stopped") << endl;
	}
	...
	void callbackSetWriteCalibImages(int state, void* data) {
			lpt::Calibrator* calibrator = static_cast<lpt::Calibrator*>(data);
			calibrator->setWriteCalibImages((state != 0));
			cout << "saving calibration images " << (state ? " started" : "stopped") << endl;
		}
