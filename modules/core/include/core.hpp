...
namespace lpt {
	...
	public:
		...
		cv::Mat image_type;
		cv::Mat half_image_type;  // the image with (640 * 512) when using MJPEG mode
	...
	public:
		typedef Object_T Object;
		//using Object = Object_T; // mahdi is equivalent
	...
	template<class Object_T>
	typename Object_T::float_type calculateDirectionalDistance(const Object_T& P1, const Object_T& P2) {
		typename Object_T::float_type distance = 0;

		// Calculate displacement in each dimension
		typename Object_T::float_type dx = P2.X[0] - P1.X[0];  // X direction
		typename Object_T::float_type dy = P2.X[1] - P1.X[1];  // Y direction
		typename Object_T::float_type dz = P2.X[2] - P1.X[2];  // Z direction

		// Penalize negative movement in the x-direction
		if (dx < -1e-6) {
			return std::numeric_limits<typename Object_T::float_type>::max();
		}

		// Optionally scale the x-direction to emphasize it
		dx *= 1.0;  // Example: Double the weight of the x-direction

		// Calculate directional distance
		distance += dx * dx;  // Weighted x-direction
		distance += dy * dy;  // Regular y-direction
		distance += dz * dz;  // Regular z-direction
	...
	inline void extrapolatePosition(lpt::Trajectory_<Object_T>& traj)
	{
		...
		for (int d = 0; d < Object_T::dim; ++d) {
			...
			//mahdi below was 120
			traj.V[d] *= 0; //FIXME: This may not be in use ....This should be varialble based on frame rate and number of frames between points -- (t2.frame_index -  t1.frame_index) );
			traj.A[d] = 0;
		}
		//for (int i = 0; i < traj.objects.rbegin()->second.size(); i++) {
		//	cout << " i= " << i << "mahdi for object t we have traj.objects.rbegin()->second= " << traj.objects.rbegin()->second[i] << endl;
		//}
		//cout << " we have V.size()= " << traj.V.size() << endl;
