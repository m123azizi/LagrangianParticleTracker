...
bool ThreePointLine::find(lpt::ImageFrame& frame) {
	...
	if (frame.particles.size() == 3) {
		...
		// Calculate the perpendicular distance from the middle point to the line formed by the other two points
		...
		// Check if the middle point is close enough to the line (collinear within a threshold)
		if ( mid_pt_dist < 1.5 )
			found = true;
		else
