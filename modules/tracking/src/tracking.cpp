// mahdi 12 / 2 / 2024 htis file was checked against douglas original file and it is the same VISUALLY. there are some differences 
// but the main functioanloty has not changed.
...
namespace lpt {
	void callbackSetAlpha(int state, void* data)
	{
		...
		cout << "setting traj alpha: " << tracker->params.alpha << endl;
	}
	...
	void callbackSetRMax(int state, void* data)
	{
		...
		cout << "setting traj max_radius: " << tracker->params.max_radius << endl;
	}
	...
	void callbackSetRMin(int state, void* data)
	{
		...
		cout << "setting traj min_radius: " << tracker->params.min_radius << endl;
	}
	...
	DirectionalCostMinAcc::DirectionalCostMinAcc()
	{
		cout << "Directional Cost Minimum Acceleration constructed" << endl;
	}

	DirectionalCostMinAcc::~DirectionalCostMinAcc()
	{
		cout << "Directional Cost Minimum Acceleration destructed" << endl;
	}

	double DirectionalCostMinAcc::calculate(const Trajectory3d& traj, const Particle3d& particle) const
	{
		std::cout << std::fixed << std::setprecision(4);
		// Calculate displacement
		//double dx = particle.X[0] - traj.extrap_object.X[0];
		double dx = particle.X[0] - (*(traj.objects.rbegin()->first)).X[0];
		// If displacement in the x-direction is negative, assign a very high cost
		if (dx < -1e-6) {
			/*cout << "Negative x-direction displacement, particle coords: " << particle.X[0] << " " << particle.X[1] << " " << particle.X[2] << 
				" last traj point: " << (*(traj.objects.rbegin()->first)).X[0] << " " << (*(traj.objects.rbegin()->first)).X[1] << " " << (*(traj.objects.rbegin()->first)).X[2] << endl;*/
			return std::numeric_limits<double>::max();
		}

		// Otherwise, proceed with normal cost calculation
		return lpt::calculateDistance(traj.extrap_object, particle);
	}
	...
	Tracker::~Tracker()
	{
		...
		if (isSave) {
			...
			int id = 0;
			size_t link_count = 0;
			cout << " size of traj " << trajectories.size() << endl;
			...
			cout << " size of active traj " << active_trajs.size() << endl;
			for (auto traj_it = active_trajs.begin(); traj_it != active_trajs.end(); traj_it++) {
				//if ((*traj_it)->objects.size() >= 10) {
					//link_count == (*traj_it)->objects.size(); 
				link_count += (*traj_it)->objects.size(); //mahdi init was: link_count == (*traj_it)->objects.size();
				saveTrajectory(output, *traj_it, id);
				id++;
				//}
			}
			cout << " size of trajectories plus actives written= " << id << endl;
			cout << "links tracked: " << link_count << endl;
			//system("pause");
			cout << " mahdi enter to close" << endl;
	...
	void Tracker::saveTrajectory(ofstream& output, Trajectory3d_Ptr traj, int id)
	{
		...
		//ofstream output(file_name.str());
		//output << fixed << setprecision(4); //mahdi
		for (size_t i = 0; i < traj->objects.size(); i++) {
			...
			for (size_t j = 0; j < traj->objects[i].first->X.size(); j++) {
				//cout << " under for (size_t j = 0; j < traj->objects[i].first->X.size(); j++) j= " << j << " , " << traj->objects[i].first->X[j] << " " << endl;
				output << traj->objects[i].first->X[j] << " ";
			}
			for (size_t j = traj->objects[i].first->X.size(); j < traj->objects[i].second.size(); j++) {
				//cout << " under for (size_t j = traj->objects[i].first->X.size(); j < traj->objects[i].second.size(); j++) { j= " << j << " , " << traj->objects[i].second[j] << " " << endl;
	...
	void Tracker::trackFrames(lpt::Frame3d& frame1, lpt::Frame3d& frame2)
	{
		...
		for (int p2 = 0; p2 < frame2.objects.size(); ++p2) {
			...
			if (traj_match) {
				...
				std::push_heap(traj_match->candidates_costs.begin(), traj_match->candidates_costs.end(), lpt::compareCost); //mahdi 11/21/24: mainitnig heap property manually, checked, it has the minimum at the front
			}
			...
			// Candidate does not exist within search radius. Loop over unmatched particles in frame 1 to find candidates within maximum radius
				...
				for (list<lpt::Particle3d_Ptr>::iterator p1_iter = unmatched_particles.begin(); p1_iter != unmatched_particles.end(); ++p1_iter) {
					...
					double cost = lpt::calculateDirectionalDistance(*(*p1_iter), *frame2.objects[p2]);
		...
		std::map<int, int> particle_claim_counts;  // To count claims for each particle
		for (list<lpt::Trajectory3d_Ptr>::iterator traj_iter = active_trajs.begin(); traj_iter != active_trajs.end(); ++traj_iter) {
			...
			if (!(*traj_iter)->candidates_costs.empty()) {
				...
				// Increment count and check for duplicates
				if (++particle_claim_counts[p2] > 1) {
					std::cout << "Duplicate claim detected for particle ID: " << p2 << std::endl;
				}
			}
			// If no candidate, remove trajectory from active list and add to trajectory list
			else {
				if ((*traj_iter)->objects.size() > 2) { //mintrajsize here was 4
	...
	void Tracker::addControls()
	{
		...
		cv::createTrackbar("Alpha", string(), &params.alpha_level, 300, callbackSetAlpha, tracker_void_ptr);//mahdi level from 200 to 300
		cv::createTrackbar("MaxRadius", string(), &params.max_radius_level, 500, callbackSetRMax, tracker_void_ptr);//mahdi level from 100 to 1100
