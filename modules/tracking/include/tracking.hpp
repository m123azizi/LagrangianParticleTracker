// mahdi 12 / 2 / 2024 htis file was checked against douglas original file and it is the same, except for two vars
//  below: size_t particle_count;size_t traj_count;
/**
 * @file tracking.hpp
...
#include "core.hpp"
#include <iomanip> //mahdi for changing precesion of out command when save trajectories is called
...
/**
 * @brief The DirectionalCostMinAcc class
	same as CostMinimumAcceleration but has the dominant flow direction impkemented in the cost calculation function
 */
class DirectionalCostMinAcc : public CostCalculator {
public:
    typedef shared_ptr<DirectionalCostMinAcc> Ptr;
    static inline DirectionalCostMinAcc::Ptr create() { return std::make_shared<lpt::DirectionalCostMinAcc>(); }

    DirectionalCostMinAcc();
    ~DirectionalCostMinAcc();

    /**
	 * @brief Calculates the candidate cost same as CostMinimumAcceleration but has the dominant flow direction
     */
    double calculate(const Trajectory3d& traj, const Particle3d& particle) const;
};
