...
namespace lpt {
	...
	void getVectorField(vector<array<lpt::boost_weighted_accumulator, 3> >& weighted_accumulators, lpt::VectorField& vector_fieldAvg, lpt::VectorField& vector_fieldMin, lpt::VectorField& vector_fieldMax, lpt::VectorField& vector_fieldVar);
	...
	*/
	...
	public:
		...
		class Parameters {
		public:
			Parameters() : traj_update_stride(0), grid_update_stride(100), queue_capacity(1000) {} //mahdi queue_capacity(500) to 1000
		...
		*/
		...
		inline void setSaveTec(bool state) {
			save_tec = state;
		...
		friend void callbackSaveTec(int state, void* data);
	...
	public:
		...
		public:
			Parameters() : mode(0), stride(2), scale(30), timer_duration(1), queue_capacity(500), min_traj_size(6) {
				//array<int, 2> size = { { 1280, 1024 } }; //mahdi
				array<int, 2> size = { { 640, 480 } };
				window_size = size;

				//array<double, 3> color = { { .1,.1,.1 } };
				array<double, 3> color = { { 201,193,181 } }; // mahdi change background color to white, milk 253,255,245
		...
		*/
		...
		/**
		added by mahdi for reading grid dimensions from file
		**/
		void readGridData(const string& gridFileAdd);
		...
		inline void setAddToRender(bool status) { addToRender_status = status; }
		...
		friend void callbackSetAddToRender(int state, void* data);
