...
// mahdi@5/17/2024: this visualization cpp file (not hpp) was compared with barker code because I had made some changes to include 
// text writing and I wanted to make sure they are not effective. the comparison result shows that they are 90% the same, except 2 differences:
// the first one is adding tecplot text writer, and the second is adding function to read grid data from input file
// other things are commented such as old writer code, or stuff for disabling neighbour update
namespace lpt {

	using namespace std;

	vtkSmartPointer<vtkLookupTable> TrajectoryPathVTK::lookuptable = 0;
	int TrajectoryPathVTK::max_points = 5; // in barker this is 10
	...
	void getVectorField(vector<array<lpt::boost_weighted_accumulator, 3> >& weighted_accumulators, lpt::VectorField& vector_fieldAvg,
		lpt::VectorField& vector_fieldMin, lpt::VectorField& vector_fieldMax, lpt::VectorField& vector_fieldVar)
	{
		vector_fieldAvg.resize(weighted_accumulators.size());
		vector_fieldMin.resize(weighted_accumulators.size());
		vector_fieldMax.resize(weighted_accumulators.size());
		vector_fieldVar.resize(weighted_accumulators.size());

		for (int p = 0; p < weighted_accumulators.size(); ++p) {
			for (int d = 0; d < 3; ++d) {
				vector_fieldAvg[p][d] = extract_result<tag::weighted_mean>(weighted_accumulators[p][d]);
				vector_fieldMin[p][d] = extract_result<tag::min>(weighted_accumulators[p][d]);
				vector_fieldMax[p][d] = extract_result<tag::max>(weighted_accumulators[p][d]);
				vector_fieldVar[p][d] = extract_result<tag::weighted_variance>(weighted_accumulators[p][d]);
			}
		}
	}
	...
	FluidProperties::FluidProperties()
		: rho(1.225), Td(15), Tw(15), mu(1.78E-5), P_ref(101325) {} // mahdi change fluid prop here
	...
	CameraVTK::CameraVTK(lpt::Camera& camera)
	{
		...
		for (int i = 0; i < 4; ++i) {
			...
			//std::cout << "mahdi this is inside visualization cpp file " << endl; //mahdi
		}
	...
	void CameraVTK::camToWorld(lpt::Camera& cam, array<double, 3>& pc, array<double, 3>& pw)
	{
		...
		//std::cout << " mahdi this is inside visualization cpp under camtoWrold" << endl; //mahdi
		//cout << " pw[0,1,2] = " << pw[0] << " /t " << pw[1] << " /t " << pw[2] << endl;
		// mahdi not printing above
	}
	...
	void FiniteVolumeGrid::initialize()
	{
		...
		////mahdi lines below might be needed
		//lookupTable->SetNanColor(1, 0, 0, 1); // Red color for NaN values
		//lookupTable->SetBelowRangeColor(0, 1, 0, 1); // Green color for values below the range
		//lookupTable->SetUseBelowRangeColor(true); // Enable the below range color
		//lookupTable->SetAboveRangeColor(0, 0, 1, 1); // Blue color for values above the range
		//lookupTable->SetUseAboveRangeColor(true); // Enable the above range color
		//// end mahdi
		...
		//mahdi change plane bounding color to black
		vtkSmartPointer<vtkProperty> outlineProperty = plane_widget->GetOutlineProperty();
		outlineProperty->SetColor(0.0, 0.0, 0.0); // RGB values for black
		//plane_widget->Modified();// Optional: Update the widget if necessary
		//end mahdi
		...
		planeWidgetX->GetPlaneProperty()->SetColor(1, 0, 0);//mahdi one color is here red
	...
	{
		//cout << " mahdi this is under update acc " << endl;
		int subIdNotUsed;
		...
		//cout << " mahdi under update U[0]= " << U[0] << " U[1]= " << U[1] << " U[2]= " << U[2] << endl;
		...
		//cout << " cell id from find_cell is: " << cell_id << endl;
		// 
		//// mahdi no neighbour update below
		...
		//if (cell_id >= 0 && cell_id < grid->GetNumberOfCells() && abs(U[0]) >= 0.0001 && abs(U[1]) >= 0.0001 && abs(U[2]) >= 0.0001) { // mahdi threshold for zero vel remove


		//mahdi comment for single cell update
		...
		if (cell_id >= 0 && cell_id < grid->GetNumberOfCells()) {
			...
			//cout << " base_cell_center[0]: " << base_cell_center[0] << endl;

			for (int n = 0; n < cell_neighborhood.size(); ++n) {
				//cout << " under neighborhood ijk base is " << ijk_base[0] << ijk_base[1] << ijk_base[2] << endl;
				array<int, 3> ijk = cell_neighborhood[n];
				//cout << " under neighborhood ijk before is " << ijk[0] << ijk[1] << ijk[2] << endl;
				ijk[0] += ijk_base[0];
				ijk[1] += ijk_base[1];
				ijk[2] += ijk_base[2];
				//cout << " under neighborhood ijk after is " << ijk[0] << ijk[1] << ijk[2] << endl;
				...
				//cout << " double distance_weight =" << distance_weight << endl;
	...
	void FiniteVolumeGrid::savePlaneData()
	{
		...
		//mahdi start
		stringstream tecHeader;
		tecHeader << "Variables = \"X\", \"Y\", \"Z\", \"VelMag\", \"X Velocity\", \"Y Velocity\", \"Z Velocity\"" <<
			", \"pres_e\", \"pres_l\", \"TKE\", \"u_variance\", \"v_variance\", \"w_variance\", \"uv_covariance\", \"uw_covariance\", \"vw_covariance\"" << endl;
		tecHeader << "ZONE I=" << this->params.grid_cell_counts[0] << ", J=" << this->params.grid_cell_counts[1] << ", K=" << this->params.grid_cell_counts[2] << ", DATAPACKING=POINT" << endl;
		//mahdi end
		...
		ofstream all_data_out(this->shared_objects->output_path + "all_data.plt");
		...
		for (int k = 0; k < this->params.grid_cell_counts[2]; ++k) {
			for (int j = 0; j < this->params.grid_cell_counts[1]; ++j) {
				for (int i = 0; i < this->params.grid_cell_counts[0]; ++i) {
					...
					all_data_out << i * this->params.cell_dimensions[0] + this->params.grid_origin[0] << "," <<
						j * this->params.cell_dimensions[1] + this->params.grid_origin[1] << "," <<
						k * this->params.cell_dimensions[2] + this->params.grid_origin[2] << "," <<
						sqrt(pow(velocities[cell_id][0], 2.0) + pow(velocities[cell_id][1], 2.0) + pow(velocities[cell_id][2], 2.0)) << "," <<
						velocities[cell_id][0] << "," << velocities[cell_id][1] << "," << velocities[cell_id][2] << "," <<
						pressures[cell_id] << "," << lagrangian_pressures[cell_id] << "," << turbulent_kinetic_energy << "," <<
						u_variance << "," << v_variance << "," << w_variance << "," <<
						uv_covariance << "," << uw_covariance << "," << vw_covariance;					
					all_data_out << endl;
	...
	void FiniteVolumeGrid::saveTecData()
	{
		lpt::VectorField velocitiesAvg;
		lpt::VectorField velocitiesMin;
		lpt::VectorField velocitiesMax;
		lpt::VectorField velocitiesVar;
		

		lpt::getVectorField(velocity_accumulators, velocitiesAvg, velocitiesMin, velocitiesMax, velocitiesVar);
		//lpt::VectorField accelerations;
		//lpt::getVectorField(acceleration_accumulators, accelerations);
		...
		//const vector<double>& pressures = pressure_solver->getPressureField();
		//const vector<double>& lagrangian_pressures = lagrangian_pressure_solver->getPressureField();
		cout << "Saving Tec plot data " << endl;
		stringstream tecHeader;
		// old tecplot header with so many unsed variables
		/*tecHeader << "Variables = \"X\", \"Y\", \"Z\", \"VelMag\", \"X Velocity\", \"Y Velocity\", \"Z Velocity\"" <<
			", \"pres_e\", \"pres_l\", \"TKE\", \"u_variance\", \"v_variance\", \"w_variance\", \"uv_covariance\", \"uw_covariance\", \"vw_covariance\", \"Count\"" << endl;*/

		tecHeader << "Variables = \"X\", \"Y\", \"Z\", \"VelMag\", \"Avg X Vel\", \"Avg Y Vel\", \"Avg Z Vel\"" <<
			", \"Min X Vel\", \"Max X Vel\", \"Var X Vel\", \"Count\"" << endl;
		tecHeader << "ZONE I=" << this->params.grid_cell_counts[0] << ", J=" << this->params.grid_cell_counts[1] << ", K=" << this->params.grid_cell_counts[2] << ", DATAPACKING=POINT" << endl;
		ofstream all_data_out(this->shared_objects->output_path + "all_data.plt");
		all_data_out << tecHeader.str();
		...
		for (int k = 0; k < this->params.grid_cell_counts[2]; ++k) {
			for (int j = 0; j < this->params.grid_cell_counts[1]; ++j) {
				for (int i = 0; i < this->params.grid_cell_counts[0]; ++i) {
					int cell_id = this->getGridIndex(i, j, k);
					// mahdi I checked count>(acceleration_accumulators and count velocity_accumulators) is equal
					size_t count_data = extract_result<tag::count>(velocity_accumulators[(int)cell_id][0]);
					/*double u_variance = stress_field[cell_id][0];
					double v_variance = stress_field[cell_id][1];
					double w_variance = stress_field[cell_id][2];
					...
					double turbulent_kinetic_energy = 1.0 / 2.0 * (u_variance + v_variance + w_variance);*/
					for (int d = 0; d < 3; d++) {
						if (velocitiesAvg[cell_id][d] != velocitiesAvg[cell_id][d]) { //checking for nan velocities
							velocitiesAvg[cell_id][d] = 0.0;
						}
						if (velocitiesMin[cell_id][d] != velocitiesMin[cell_id][d]) { //checking for nan velocities
							velocitiesMin[cell_id][d] = 0.0;
						}
						if (velocitiesMax[cell_id][d] != velocitiesMax[cell_id][d]) { //checking for nan velocities
							velocitiesMax[cell_id][d] = 0.0;
						}
						if (velocitiesVar[cell_id][d] != velocitiesVar[cell_id][d]) { //checking for nan velocities
							velocitiesVar[cell_id][d] = 0.0;
						}
					}

					double velocityMagnitude = sqrt(pow(velocitiesAvg[cell_id][0], 2.0) + pow(velocitiesAvg[cell_id][1], 2.0) + pow(velocitiesAvg[cell_id][2], 2.0));
					all_data_out << i * this->params.cell_dimensions[0] + this->params.grid_origin[0] << "," <<
						j * this->params.cell_dimensions[1] + this->params.grid_origin[1] << "," <<
						k * this->params.cell_dimensions[2] + this->params.grid_origin[2] << "," <<
						velocityMagnitude << "," <<
						velocitiesAvg[cell_id][0] << "," << velocitiesAvg[cell_id][1] << "," << velocitiesAvg[cell_id][2] << "," <<
						velocitiesMin[cell_id][0] << "," << velocitiesMax[cell_id][0] << "," << velocitiesVar[cell_id][0] << "," << count_data;
						/*pressures[cell_id] << "," << lagrangian_pressures[cell_id] << "," << turbulent_kinetic_energy << "," <<
						u_variance << "," << v_variance << "," << w_variance << "," <<
						uv_covariance << "," << uw_covariance << "," << vw_covariance << "," << count_data;*/
					all_data_out << endl;
		...
		all_data_out.close();
		cout << "Done Saving Tec data" << endl;
	}
	...
	TrajectoryHandler::TrajectoryHandler(vtkSmartPointer<vtkRenderer> renderer)
		...
		remove_traj_view(false), add_cameras_view(true), remove_cameras_view(false), save_tec(false), view_mode(lpt::TRAJECTORIES)
	{
	}
	...
	void TrajectoryHandler::Execute(vtkObject* caller, unsigned long eventId, void* vtkNotUsed(callData))
	{
		...
		//textActor->GetTextProperty()->SetColor(1, 0, 0); // Set the text color to red
		//textActor->SetDisplayPosition(10, 10); // Position the text
	...
	void TrajectoryHandler::addControls()
	{
		...
		cv::createButton("Save TecPlot", callbackSaveTec, trajhandler_void_ptr, cv::QT_PUSH_BUTTON, 0);
	...
	void callbackSaveTec(int state, void* data)
	{
		TrajectoryHandler* traj_handler = static_cast<TrajectoryHandler*>(data);
		traj_handler->setSaveTec(true);
	}
	...
	Visualizer::Visualizer()
		: visualization_status(false), accumulate_status(false), addToRender_status(true),
		take_measurement(false), image_measurement(false), measurement_count(0)
	{
		cout << " mahdi this is under Visualizer::Visualizer() \n";
	...
	void Visualizer::addControls()
	{
		...
		cv::createButton("Run Visualization", callbackSetVisualizationStatus, visualizer_void_ptr, cv::QT_CHECKBOX, 1); //mahdi run visualization 1
		cv::createButton("Accumulate", callbackSetAccumulate, visualizer_void_ptr, cv::QT_CHECKBOX, 0);
		cv::createButton("addToRender", callbackSetAddToRender, visualizer_void_ptr, cv::QT_CHECKBOX, 1);
	...
	void Visualizer::manageQueue()
	{
		...
		while (true) {
			...
			// code below many print 0 becuase sometimes no trajectory is pushed to traj_updates
			//cerr << " item picked from trajQ under manageQueue# size of traj_updates " << traj_updates.size() << endl;
			...
			for (auto traj_iter = traj_updates.begin(); traj_iter != traj_updates.end(); ++traj_iter, ++id) {
				...
				//cout << "object_iter->first->frame_index " << object_iter->first->frame_index << endl;
				...
				if (this->shared_objects->KF_isOn) {
					...
					X0 = iter_previous->first->X; // previous particle, index 0, each X holds 3 x y z vals
					...
					for (int d = 0; d < X0.size(); ++d) {
						...
						U1[d] = ((X2[d] - X0[d]) * frame_rate) / 2.0 / 1000.0;                      // Velocity //velocity is calculated here mahdi						
						object_iter->second.at(d + 3) = U1[d] * 1000.0;
						//cout << " mahdi under vis., d=" << d << ", X2[d]=" << X2[d] << ", X1[d]=" << X1[d] << ",U1[d]=" << U1[d] << endl;
						A[d] = (X2[d] - 2.0 * X1[d] + X0[d]) * frame_rate * frame_rate / 1000.0;     // Acceleration
						object_iter->second.at(d + 6) = A[d] * 1000.0;
					}
					//mahdi how many objects are in this trajectory
					//cerr << " how many objects are in this trajectory " << current_traj->objects.size() << endl;
					...
					//mahdi comments: U1 is the velcoity of current trajectory based on 2nd order FD
					// U0 is similar but for the previous point
					// U2 is similar but fot the next point
				}
				...
				for (int d = 0; d < X1.size(); ++d) {
					...
					//cout << " mahdi under make new_vectors we have U1[d]= " << U1[d] << endl;
					new_vectors[2][d] = A[d];           // Acceleration
					new_vectors[3][d] = 0.0;//dPdX[d];		// Pressure gradient
				}
			...
			if (count >= params.stride && addToRender_status) {
	...
	void Visualizer::addTrajectoriesToQueue(list<lpt::Trajectory3d_Ptr>& active_trajs)
	{
		...
		if (!traj_queue.full()) {
			...
			for (traj_iter = active_trajs.begin(); traj_iter != active_trajs.end(); ++traj_iter) {
				...
				////mahdi 11/21/24 printing frame index of all objects in the trajectory
				//cerr << "Trajectory with " << current_traj->objects.size() << " particles:" << endl;
				//for (auto obj_iter = current_traj->objects.begin(); obj_iter != current_traj->objects.end(); ++obj_iter) {
				//	if (obj_iter->first) { // Ensure the particle pointer is not null
				//		cerr << "Particle frame_index: " << obj_iter->first->frame_index << endl;
				//	}
				//	else {
				//		cerr << "Null particle encountered!" << endl;
				//	}
				//}
				////end of mahdi 11/21/24 printing frame index of all objects in the trajectory
				if (current_traj->objects.size() >= 3) { // mahdi constant variable here was >3 not =
					//if (current_traj->objects.size() >= params.min_traj_size) {
						auto current_object_iter = current_traj->objects.end() - 2; //was 3
						traj_updates.push_back(std::move(make_pair(current_traj, current_object_iter)));
						//cerr << " item pushed to traj_updates under addTrajectoriesToQueue and it has (particles) size of " << current_traj->objects.size() << endl;
					//}
				}
			}
		}
		// code below many print 0 becuase sometimes no trajectory is pushed to traj_updates
		//cerr << " item pushed in trajQ under addTrajectoriesToQueue and it has size of " << traj_updates.size() << endl;
		...
		if (particle_vectors.size() > 1) {
			...
			for (int i = 0; i < particle_vectors.size() - 1; ++i) {
				for (int j = i + 1; j < particle_vectors.size(); ++j) {
					...
					cout << " mahdi under take measurment dist = " << dist << endl;
					if (dist > 15 && dist < 25) {
	...
	void Visualizer::readGridData(const string& filename) {
		double grid_size[3];
		double grid_origin[3];
		int cell_counts[3];
		lpt::readGridFile(filename, grid_size, cell_counts, grid_origin);

		this->getVolumeGrid()->setGridDimensions(grid_size[0], grid_size[1], grid_size[2]);
		this->getVolumeGrid()->setGridCellCounts(cell_counts[0], cell_counts[1], cell_counts[2]);
		this->getVolumeGrid()->setGridOrigin(grid_origin[0], grid_origin[1], grid_origin[2]);
		cout << " done reading and setting grid dimensions" << endl;
	}
	...
	void callbackSetAccumulate(int state, void* data)
	{
		...
		cout << " Accumulate " << (state ? " on " : " off ") << endl;
	}

	void callbackSetAddToRender(int state, void* data)
	{
		Visualizer* visualizer = static_cast<Visualizer*>(data);
		visualizer->setAddToRender((state != 0));
		cout << " SetAddToRender " << (state ? " on " : " off ") << endl;
	}
